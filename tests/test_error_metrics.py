...
from auto_bots.utils.error_metrics import mase, mse
...
def test_mase(tolerance: float = 0.01):
    ...
    try:
        ...
        raise AssertionError(
            f"MASE test failed: Difference between {correct_error} and {test_error} " f"is greater than {tolerance}"
        )
...
def test_mse(tolerance: float = 0.01):
    ...
    try:
        ...
        raise AssertionError(
            f"MSE test failed: Difference between {correct_error} and {test_error} " f"is greater than {tolerance}"
        )
